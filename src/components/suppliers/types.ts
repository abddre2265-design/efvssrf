export type SupplierType = 'individual_local' | 'business_local' | 'foreign';
...
// Identification types per supplier type
export const SUPPLIER_IDENTIFIER_TYPES = {
  individual_local: ['cin', 'passport', 'tax_id'],
  business_local: ['tax_id'],
  foreign: [
    'passport', 'tax_id', 'ssn', 'vat_eu', 'business_number_ca',
    'trade_register', 'national_id', 'diplomatic_passport', 'internal_id'
  ]
} as const;
...
// All countries for foreign suppliers
...
export const getIdentifierValidation = (identifierType: string, value: string, isRequired: boolean): { valid: boolean; message?: string } => {
  if (!value) {
    if (isRequired) return { valid: false, message: 'required' };
    return { valid: true };
  }
