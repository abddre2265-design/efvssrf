...
export const ClientCreateDialog: React.FC<ClientCreateDialogProps> = ({
  ...
  const initialData: Partial<ClientFormData> | undefined = duplicateFrom ? {
    clientType: duplicateFrom.client_type,
  ...
  const handleSubmit = async (data: ClientFormData) => {
    ...
    try {
      ...
      const clientData = {
        ...
        client_type: data.clientType as 'individual_local' | 'business_local' | 'foreign',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh]" dir={isRTL ? 'rtl' : 'ltr'}>
        <DialogHeader>
          <DialogTitle>
            {duplicateFrom ? t('duplicate_client') : t('create_client')}
          </DialogTitle>
        </DialogHeader>
