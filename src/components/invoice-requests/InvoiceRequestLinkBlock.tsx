...
import { InvoiceRequestLink } from './types';

export const InvoiceRequestLinkBlock: React.FC = () => {
  const { t } = useLanguage();
  const [linkData, setLinkData] = useState<InvoiceRequestLink | null>(null);
  ...
  const fetchLinkData = async () => {
    ...
    try {
      ...
      if (data) {
        setLinkData(data as InvoiceRequestLink);
  ...
  const copyLink = async () => {
    ...
    const url = `${window.location.origin}/invoice-request/${linkData.access_token}`;
  ...
  const getPublicUrl = () => {
    if (!linkData) return '';
    return `${window.location.origin}/invoice-request/${linkData.access_token}`;
  };
    ...
    <>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-lg">{t('invoice_request_link')}</CardTitle>
                <CardDescription>{t('invoice_request_link_description')}</CardDescription>
              </div>
            </div>
            {linkData && (
        ...
        <CardContent className="space-y-4">
          {linkData ? (
            ...
            <div className="text-center py-4">
              <p className="text-muted-foreground mb-4">{t('no_invoice_request_link_configured')}</p>
              <Button onClick={() => {
                setAccessCode(generateCode());
                setShowSettings(true);
              }}>
                <Link2 className="h-4 w-4 mr-2" />
                {t('create_invoice_request_link')}
              </Button>
            </div>
          )}
        </CardContent>
      ...
      <Dialog open={showSettings} onOpenChange={setShowSettings}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{linkData ? t('edit_link_settings') : t('create_invoice_request_link')}</DialogTitle>
            <DialogDescription>
              {t('invoice_request_link_settings_description')}
            </DialogDescription>
          </DialogHeader>
